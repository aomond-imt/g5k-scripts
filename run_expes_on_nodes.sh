#!/bin/bash

echo "reset in_progress"
helper_scripts/reset_sweeper_inprogress.sh

hosts=$(echo $(oarstat -fu --json) | jq -r '.[].assigned_network_address[]')
for host in $hosts; do
  host_split=($(echo "$host" | tr "." " "))
  host_node=${host_split[0]}
  echo "starting on host $host_node"
  tmux kill-session -t "$host_node"
  tmux new-session -d -s "$host_node" "ssh -t $host_node 'cd ~/journal-pdc-experiments; source venv/bin/activate; export PYTHONPATH=$PYTHONPATH:$(pwd); python3 run_experiments.py'"
done
